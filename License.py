...
def segment_chars(plate_img, fixed_width):
    """
    Trích xuất giá trị HSV từ hình ảnh, phân ngưỡng và tìm ra các ký tự trên biển số
    """
...
class PlateFinder:
    def __init__(self):
        self.min_area = 4000  # minimum area of the plate
        self.max_area = 8000  # maximum area of the plate

        self.element_structure = cv2.getStructuringElement(shape=cv2.MORPH_RECT, ksize=(22, 3))

    def preprocess(self, input_img):
        '''
         Xử lý hình ảnh thành phân ngưỡng, hình ảnh trả về là phân ngưỡng trắng đen
        '''
    ...
    def extract_contours(self, after_preprocess):
        '''
        trích đường viền ngoài của ảnh vừa mới xử lý ở (def preprocess) trên
        '''
    ...
    def find_possible_plates(self, input_img):
        """
        Tìm tất cả các contour có khả năng là biển số
        """
        plates = []
        self.char_on_plate = []
    ...
    def ratioCheck(self, area, width, height):
        '''
        Xét xem thử có phải là hình dạng biển số không, nếu đúng tỉ lệ thì trả về true còn sai thì false
        '''
        min = self.min_area
        max = self.max_area
    ...
    def preRatioCheck(self, area, width, height):
        '''
        Cũng là xét xem có phải biển số không nhưng với tỉ lệ khác
        '''
        min = self.min_area
        max = self.max_area
    ...
    def validateRatio(self, rect):
        '''
        Xác thực tỉ lệ của biển số: nếu biển số nghiên 1 góc hơn 15 độ thì sẽ không nhận dạng được
        '''
    ...
    def clean_plate(self, plate):
        '''
        Tìm giá trị toạ độ của đường bao xung quanh biển số
        '''
        ...
        if contours:
            ...
            x, y, w, h = cv2.boundingRect(max_cnt) #tìm toạ độ của những hình chữ nhật đứng
            rotatedPlate = plate
    ...
    def check_plate(self, input_img, contour):
        ...
        if self.validateRatio(min_rect):
            ...
            if plateFound:
                ...
                if (characters_on_plate is not None and 7 <= len(characters_on_plate) >= 8):
...
class OCR:
    def __init__(self):
        self.model_file = "./model/train.pb"
        self.label_file = "./model/text.txt"
